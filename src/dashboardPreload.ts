...
function listenTuple<T extends unknown[]>(channel: string, callback: (...args: T) => void): Cleanup {
...
contextBridge.exposeInMainWorld('dashboardAPI', {
  getInitialAgents: () => requestResponse<unknown[]>('get-dashboard-agents', 'dashboard-agents-response'),
  onInitialData: (callback: (data: unknown) => void) => listen('dashboard-initial-data', callback),
  onAgentAdded: (callback: (data: unknown) => void) => listen('dashboard-agent-added', callback),
  onAgentUpdated: (callback: (data: unknown) => void) => listen('dashboard-agent-updated', callback),
  onAgentRemoved: (callback: (data: unknown) => void) => listen('dashboard-agent-removed', callback),
  ...
  createRegisteredAgent: (data: Record<string, unknown>) => ipcRenderer.invoke('registry:create', data),
  inspectWorkspaceRepo: (repoPath: string) => ipcRenderer.invoke('workspace:inspect-repo', repoPath),
  createWorkspaceAgent: (data: Record<string, unknown>) => ipcRenderer.invoke('workspace:create', data),
  ...
  updateRegisteredAgent: (id: string, fields: Record<string, unknown>) =>
  ...
  getConversation: (registryId: string, sessionId: string, options?: Record<string, unknown>) =>
  ...
  createTerminal: (agentId: string, options?: Record<string, unknown>) =>
