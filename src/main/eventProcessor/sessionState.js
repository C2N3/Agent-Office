...
function createSessionState({ agentManager, agentRegistry, sessionPids, debugLog, logPrefix = 'Event', updateSource = 'event', }) {
    ...
    function canBindRegistryAgent(registryAgent) {
        if (!registryAgent || !registryAgent.id)
            return false;
        if (registryAgent.currentSessionId)
            return false;
    ...
    function adoptSessionIdentity(previousSessionId, nextSessionId) {
        ...
        if (!previousCanonical || !nextCanonical)
            return nextCanonical || previousCanonical || null;
        if (previousCanonical === nextCanonical) {
        ...
        if (registryId) {
            ...
            agentRegistry?.replaceSessionId?.(registryId, previousCanonical, nextCanonical, sessionContext.get(nextCanonical)?.meta?.jsonlPath || null, {
