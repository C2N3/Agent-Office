...
export function createEventHandlers({
  ...
  const {
    ...
    enqueueSessionStart,
    flushPendingStarts: flushPendingStartsFromState,
    firstToolUseDone,
    sessionToRegistry,
  ...
  function handleSessionStart(sessionId: string, cwd: string, pid = 0, options: Partial<PendingSessionStart> = {}) {
    ...
    detectPidByTranscript((resolvedMeta.jsonlPath as string | null) || null, (result) => {
      if (!result) return;
      if (typeof result === 'number') {
        sessionPids.set(sessionId, result);
  ...
  function processEvent(event: EventLike) {
    if (!event) return;
    ...
    switch (event.type) {
      case 'session.start': {
        ...
        if (sessionSource !== 'startup' && agentManager) {
          ...
          if (existing) {
            const compactUpdate: UpdateAgentPayload = {
              ...existing,
              sessionId,
  ...
  function attachRegisteredAgent(registryAgent: AgentLike | null | undefined) {
