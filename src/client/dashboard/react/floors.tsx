...
export function FloorTabs({
  ...
  return (
    <>
      ...
      <button className="floor-tab-add" id="floorAddBtn" title="Add Floor" type="button" onClick={onOpenCreate}>+</button>
      <button className="floor-tab-manage" id="floorManageBtn" title="Manage Floors" type="button" onClick={onOpenManage}>
        ...
        <div className="floor-create-overlay" id="floorCreateOverlay" onClick={(event) => {
          ...
          <div className="floor-create-dialog">
            ...
            <div className="floor-create-actions">
              <button className="floor-cancel-btn" id="floorCancelBtn" type="button" onClick={onCloseDialog}>Cancel</button>
              <button className="floor-confirm-btn" id="floorConfirmBtn" type="button" onClick={onCreateFloor}>Create</button>
            </div>
          </div>
        </div>
      ) : null}
      {dialog === 'manage' ? (
        <div className="floor-create-overlay" id="floorManagerOverlay" onClick={(event) => {
          ...
          <div className="floor-create-dialog floor-mgr-dialog">
            ...
            <div className="floor-mgr-body">
              {floors.map((floor) => (
                <div key={floor.id} className="floor-mgr-row" data-floor-id={floor.id}>
                  ...
                  />
                  <span className="floor-mgr-count">{floor.agentIds.length} agents</span>
                  {floor.id === currentFloorId ? <span className="floor-mgr-current">current</span> : null}
                  {floors.length > 1 ? (
                    <button
            ...
            <div className="floor-create-actions">
              <button className="floor-confirm-btn" id="floorMgrAddBtn" type="button" onClick={onOpenCreate}>+ Add Floor</button>
            </div>
          </div>
        </div>
      ) : null}
