...
export function parsePathListValue(inputId) {
  return String((document.getElementById(inputId) as HTMLInputElement | HTMLTextAreaElement | null)?.value || '')
    .split(/\r?\n|,/)
...
export function updateWorktreeFieldState(worktreeFields, registrationPreview, strategyValue) {
  ...
  Array.from(worktreeFields.querySelectorAll('input, textarea, button, select')).forEach((element) => {
    const field = element as HTMLInputElement | HTMLTextAreaElement | HTMLButtonElement | HTMLSelectElement;
    field.disabled = !worktreeEnabled;
  });
}
...
export async function submitAgentCreateForm({
  ...
  const dashboardAPI = getDashboardAPI();
  const name = (document.getElementById('agentNameInput') as HTMLInputElement | null)?.value.trim() || '';
  const role = (document.getElementById('agentRoleInput') as HTMLInputElement | null)?.value.trim() || '';
  if (!name) {
  ...
  const result = await dashboardAPI.createAgentFromPath({
    ...
    bootstrapCommand: (document.getElementById('agentBootstrapCommandInput') as HTMLInputElement | null)?.value.trim() || '',
  });
  if (!result?.success) {
  ...
  const shouldOpenTerminal = !!(document.getElementById('workspaceOpenTerminalInput') as HTMLInputElement | null)?.checked;
  closeModal?.();
  resetFormState?.();
