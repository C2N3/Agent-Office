...
export function setupConversationViewer(resumeRegisteredSession) {
  ...
  const sessionListEl = modal.querySelector('.conv-session-list') as HTMLElement | null;
  const chatPanel = modal.querySelector('.conv-chat-panel') as HTMLElement | null;
  const chatMessages = modal.querySelector('.conv-chat-messages') as HTMLElement | null;
  const chatSessionId = modal.querySelector('.conv-chat-session-id') as HTMLElement | null;
  const backBtn = modal.querySelector('.conv-back-btn') as HTMLButtonElement | null;
  const resumeBtn = modal.querySelector('.conv-resume-btn') as HTMLButtonElement | null;
  const closeBtn = modal.querySelector('.conv-modal-close') as HTMLButtonElement | null;
  const titleEl = modal.querySelector('.conv-modal-title') as HTMLElement | null;
  ...
  async function openSessionHistory(registryId, agentName) {
    ...
    try {
      ...
      sessionListEl.querySelectorAll<HTMLElement>('.conv-session-item').forEach((item) => {
