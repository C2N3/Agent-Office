...
export function setupAgentModal(openTerminalForAgent) {
  ...
  const agentNameInput = document.getElementById('agentNameInput') as HTMLInputElement | null;
  ...
  const providerBtns = document.querySelectorAll<HTMLElement>('#providerSelect .provider-btn');
  ...
  function fallbackBranchName() {
    return buildFallbackBranchName(
      agentNameInput?.value.trim() || 'agent',
