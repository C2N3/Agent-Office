// @ts-nocheck
import { DOM, archiveState, getDashboardAPI, state, } from './shared.js';
import { clearUnregisteredAgents, connectSSE, initFilterControls, updateAgentUI, updateBulkArchiveButton, } from './agentViews.js';
import { initViewControls, renderArchiveView, renderHeatmapView, renderUsageView, } from './activityViews.js';
import { setupOfficeClickHandler } from './office.js';
import { fitActiveTerminal, initResizableHandles, initTerminalProfileMenu, initTerminals, openTerminalForAgent, refreshTerminalProfiles, resumeRegisteredSession, } from './terminal.js';
import { setupAgentModal, setupAvatarPicker, setupConversationViewer, setupNicknameEdit, } from './modals.js';
function initPipControls() {
    ...
    function setPipState(isOpen) {
        ...
        if (pipPlaceholder)
            pipPlaceholder.style.display = isOpen ? 'flex' : 'none';
        if (officeCanvas)
            officeCanvas.style.display = isOpen ? 'none' : 'block';
    }
...
function initInitialView() {
    ...
    if (!button)
        button = document.querySelector('[data-view="office"]');
...
function initAgentPanelEvents() {
    ...
    agentPanel.addEventListener('click', (event) => {
        ...
        if (event.target.closest('.nickname-input') || event.target.closest('.agent-display-name'))
            return;
