// @ts-nocheck
import { SHARED_AVATAR_FILES, escapeText, getDashboardAPI, state, } from './shared.js';
...
export function setupAgentModal(openTerminalForAgent) {
    ...
    if (!modal || !form || !openBtn || !existingFields || !worktreeFields)
        return;
    ...
    function resetFormState() {
        ...
        if (inspectStatusEl)
            inspectStatusEl.textContent = 'Enter a repository path to inspect branches.';
        if (errorEl)
            errorEl.textContent = '';
        const openTerminalCheckbox = document.getElementById('workspaceOpenTerminalInput');
        if (openTerminalCheckbox)
            openTerminalCheckbox.checked = true;
    }
    function closeModal() {
    ...
    function updateBranchModeLabel() {
        if (branchModeInput)
            branchModeInput.value = branchMode === 'auto' ? 'Auto' : 'Custom';
    }
    ...
    function syncAutoBranch() {
        if (branchMode !== 'auto' || !branchInput)
            return;
        branchInput.value = suggestBranchName();
    }
    function syncStartPointToBaseBranch() {
        if (startPointTouched || !startPointInput || !baseBranchInput)
            return;
    ...
    async function inspectRepository(repoPath) {
        ...
        if (trimmedPath === lastInspectedRepoPath)
            return;
        lastInspectedRepoPath = trimmedPath;
        repoInspection = null;
        if (inspectStatusEl)
            inspectStatusEl.textContent = 'Inspecting repository...';
        const dashboardAPI = getDashboardAPI();
        if (!dashboardAPI?.inspectWorkspaceRepo) {
            if (inspectStatusEl)
                inspectStatusEl.textContent = 'Repository inspection is only available in the Electron app.';
            return;
        }
        ...
        if (!result?.success) {
            ...
            populateBaseBranchOptions([]);
            if (inspectStatusEl)
                inspectStatusEl.textContent = result?.error || 'Could not inspect repository.';
            return;
        }
    ...
    repoPathInput?.addEventListener('input', () => {
        ...
        inspectTimer = setTimeout(() => {
            inspectRepository(repoPathInput.value).catch((error) => {
                ...
                if (inspectStatusEl)
                    inspectStatusEl.textContent = 'Could not inspect repository.';
            });
        }, 300);
    });
    ...
    form.addEventListener('submit', async (event) => {
        ...
        if (createMode === 'worktree') {
            ...
            if (!dashboardAPI?.createWorkspaceAgent) {
                if (errorEl)
                    errorEl.textContent = 'Workspace creation is not available.';
                return;
            }
            const payload = {
                name,
            ...
            if (!result?.success) {
                if (errorEl)
                    errorEl.textContent = result?.error || 'Failed to create workspace.';
                return;
            }
        ...
        if (!projectPath) {
            if (errorEl)
                errorEl.textContent = 'Project path is required.';
            return;
        }
...
export function setupConversationViewer(resumeRegisteredSession) {
  ...
  `;
    ...
    resumeBtn.addEventListener('click', async () => {
        if (!currentRegistryId || !(currentResumeSessionId || currentSessionId))
            return;
        const dashboardAPI = getDashboardAPI();
