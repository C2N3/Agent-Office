// @ts-nocheck
import { dashboardResumeUtils, escapeText, getDashboardAPI, state, termState, } from './shared.js';
...
export async function refreshTerminalProfiles() {
    const dashboardAPI = getDashboardAPI();
    if (!dashboardAPI?.getTerminalProfiles)
        return;
...
async function ensureTerminalProfilesLoaded() {
    if (termState.profiles.length > 0)
        return;
...
export async function openTerminalForAgent(agentId, openOptions = {}) {
    ...
    if (shouldAutoResume) {
        const resumeResult = await resumeLatestRegisteredSession(registryId, openOptions.label || agent?.nickname || agent?.name || 'Terminal');
    ...
    if (!dashboardAPI?.createTerminal)
        return;
