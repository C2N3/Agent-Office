// @ts-nocheck
import { DOM, archiveState, createDiv, escapeText, formatDateTime, formatNum, getDashboardAPI, historyState, state, } from './shared.js';
const MODEL_COLORS = {
    opus: '#e879a0',
    sonnet: '#2f81f7',
    haiku: '#3fb950',
  ...
  `;
    ...
    const getLevel = (value) => {
        if (value === 0 || nonZeroValues.length === 0)
            return 0;
        if (value <= nonZeroValues[Math.floor(nonZeroValues.length * 0.25)] || 1)
            return 1;
        if (value <= nonZeroValues[Math.floor(nonZeroValues.length * 0.5)] || 1)
            return 2;
        if (value <= nonZeroValues[Math.floor(nonZeroValues.length * 0.75)] || 1)
            return 3;
        return 4;
    };
