// @ts-nocheck
import { DOM, REGISTERED_FILTER_STORAGE_KEY, SHARED_AVATAR_FILES, state, escapeText, formatNum, getDashboardAPI, } from './shared.js';
let sseDelay = 1000;
let sseSource = null;
...
export async function clearUnregisteredAgents() {
    ...
    if (!confirm(`Clear ${count} inactive unregistered agent${count === 1 ? '' : 's'}?`))
        return;
    const dashboardAPI = getDashboardAPI();
    if (!dashboardAPI?.clearInactiveUnregisteredAgents)
        return;
    if (DOM.bulkArchiveBtn)
        DOM.bulkArchiveBtn.disabled = true;
    let clearedCount = 0;
    try {
