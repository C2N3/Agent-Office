...
- the terminal container and empty-state hosts now come from React-owned `TerminalPanel` refs instead of `document.getElementById('terminalContainer'/'terminalEmptyState')`, while xterm/task-log creation stays imperative
...
- React-owned terminal container and empty-state host registration for xterm/task-log creation, with terminal rendering still managed by the existing imperative adapter
...
- continue auditing any newly discovered React-rendered dashboard controls for follow-up `getElementById(...).addEventListener(...)` wiring; the latest audit left dashboard root mounting and office canvas/popover adapters outside the xterm-host slice
...
1. Continue dashboard adapter cleanup for remaining React-rendered DOM lookups, moving next to office canvas/popover adapters only where a small owner can be identified.
