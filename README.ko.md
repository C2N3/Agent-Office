...
같은 카드의 `Sync agent characters through this server`를 켜면 새로 등록한 대시보드 agent, 아바타 변경, archive/delete 동작이 설정된 중앙 서버로 동기화됩니다. 동기화 토글은 `~/.agent-office/central-agent-sync.txt`에 저장됩니다. 꺼져 있을 때는 agent character 생성과 수정이 기존처럼 `~/.agent-office/agent-registry.json`에만 남습니다.
