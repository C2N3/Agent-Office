...
>
> 현재 프로덕션 런타임 산출물은 `dist/` 기준입니다. `npm start`, `npm run dashboard`, `npm test`는 실행 전에 자동으로 `npm run build:dist`를 호출합니다. `npm run dev`는 source 변경을 감지해 `dist/`를 다시 빌드한 뒤 Electron을 자동 재시작합니다. `node dist/...` 경로를 직접 실행할 때는 먼저 `npm run build:dist`를 한 번 돌려 두세요.
