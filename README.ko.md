...
같은 탭에서 이제 `Local Only`, `Host`, `Guest` 세 가지 모드를 직접 선택할 수 있습니다.

- `Local Only`는 중앙 서버 주소만 유지하고 worker bridge와 character sync는 끕니다.
- `Host`는 이 PC의 worker bridge와 character sync를 함께 켜고, 필요하면 worker token을 사용하며, room-access 제어를 통해 guest invite link를 발급할 수 있게 합니다.
- `Guest`는 invite link를 받아 guest room secret을 로컬에 저장한 뒤, 그 secret으로 worker bridge와 character sync를 켭니다.
